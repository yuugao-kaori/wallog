...
const BlogPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch('/api/blog2/blog_create', {
        method: 'POST',
        headers: {
  ...
  const fetchBlogs = async (page: number) => {
    try {
      ...
      const response = await fetch(`/api/blog2/blog_list?offset=${offset}&limit=${ITEMS_PER_PAGE}`);
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-6 max-w-2xl mx-auto"> {/* コンテナを中央寄せし、最大幅を設定 */}
            {blogs.map((blog: any) => (
              <Link href={`/blog2/${blog.blog_id}`} key={blog.blog_id}>
